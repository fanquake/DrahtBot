...
fn calc_coverage(
    ...
    docker_exec(&format!("apt-get install -qq {}", "python3-zmq libsqlite3-dev libevent-dev libboost-dev libcapnp-dev capnproto libzmq3-dev lcov build-essential cmake pkg-config"));
...
#[derive(clap::Parser)]
#[command(about = "Create Bitcoin Core coverage reports.", long_about = None)]
struct Args {
